...
class ResidentEvil3Remake(World):
    ...
    apworld_release_version = "0.1.2" # defined to show in spoiler log
    ...
    def create_items(self):
        ...
        if len(traps) > 0:
            ...
            available_spots = [
                item for item in pool 
                    if 'Explosive' in item.name or 'Gunpowder' in item.name or ('Film' in item.name and 'Hiding Place' not in item.name)
            ]
            ...
            extra_spots = [
                item for item in pool 
                    if 'Rifle Ammo' in item.name or item.name == 'Handgun Ammo'
            ]
        ...
        if missing_item_count > 0:
            for x in range(missing_item_count):
                pool.append(self.create_item('Assault Rifle Ammo'))
